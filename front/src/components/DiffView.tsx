...
function FileBlock({ file, view }: { file: FileData; view: ViewType }) {
  ...
  return (
    <div className="border border-line rounded-md overflow-hidden bg-surface">
      <div className="px-3 py-2 border-b border-line bg-surface-muted text-[12px] font-mono truncate flex items-center gap-2">
        <span className="text-fg">{title}</span>
        {file.oldPath && file.newPath && file.oldPath !== file.newPath && (
          <span className="text-fg-faint text-[11px]">← {file.oldPath}</span>
        )}
        <span className="ml-auto text-[10px] uppercase tracking-wider text-fg-faint">
          {file.type}
        </span>
      </div>
...
function UnifiedRaw({ text }: { text: string }) {
  return (
    <pre className="text-[12px] leading-[1.5] font-mono whitespace-pre overflow-x-auto p-3 bg-surface border border-line rounded-md">
      {text.split("\n").map((l, i) => {

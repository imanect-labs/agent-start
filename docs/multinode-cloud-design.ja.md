...
> Status: **Phase 0〜2 実装済み** / 2026-08
> 実装状況は §5 のロードマップに反映（Phase 0 / 1 / 2 = 完了）。
...
| 論点 | 当初案 | 実装 | 理由 |
| --- | --- | --- | --- |
| キューの排他 | `SELECT … FOR UPDATE SKIP LOCKED` | 「pending のままなら UPDATE」の条件付き更新（`rows_affected == 1` が獲得） | SQLite に `SKIP LOCKED` が無い。意味論は同じで、Phase 4 の Postgres 実装は呼び出し側を変えずに差し替えられる |
...
**再実行の安全性**: push か PR 作成に到達したタスクは `side_effects_committed` が立ち、
以後は自動再キューされない（1 依頼に 2 つの PR が生えるのを防ぐ）。
手動 `retry` は attempts をリセットするが、このフラグは残す。
